...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999):
  ...
  port=3008
  client = snakeoil3.Client(p=3008, vision=False)  # Open new UDP in vtorcs
...
def master():
  ...
  while True:
    t += 1
    print("EWDFEWCFWEFEWCFERCFFERWCGFWERCGRTGEVEEGTTTTTTTTTTTTTTTTTTTTTTTTTTTT")
    solutions = es.ask()
    if antithetic:
      seeds = seeder.next_batch(int((es.popsize+1)/2))
      seeds = seeds+seeds
    else:
    ...
    result = []
    worker_num = 0
    worker_num = int(0 / num_worker_trial) + 1
    result.append(worker_num)
    result.append(0)
    result.append(seeds[0])
    result.append(1)
    result.append(max_len)
   # print(result)
    result.append(np.round(np.array(solutions[0]),0))
    #print(result)
    #result = np.concatenate(result).astype(np.int32)
    #print(result)
    #result = np.split(result, num_worker)
    ...
    worker_id, jobidx, seed, train_mode, max_len, weights = result
    jobidx = int(jobidx)
    seed = int(seed)
    fitness, timesteps = worker(weights, seed, 1, max_len)
    results.append([worker_id, jobidx, fitness, timesteps])
...
if __name__ == "__main__":
  ...
  if "parent" == mpi_fork(args.num_worker): os.exit()
  main(args)
