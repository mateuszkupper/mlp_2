...
def random_batch():
  ...
  s = logvar.shape
  z = mu + np.exp(logvar/10.0) * np.random.randn(*s)
  return z, action

def default_hps():
  return HyperParams(num_steps=4000,
                     max_seq_len=999, # train on sequences of 1000 (so 999 + teacher forcing shift)
                     input_seq_width=19,    # width of our data (32 + 3 actions)
                     output_seq_width=16,    # width of our data is 32
                     rnn_size=256,    # number of rnn cells
                     batch_size=40,   # minibatch sizes
                     grad_clip=1.0,
...
for local_step in range(hps.num_steps):
  ...
  temp_a = []
  temp_z = []
  for a, z in zip(raw_a, raw_z):
    if len(a.tolist())!=1000:
      continue
    temp_a.append(a.tolist())
    temp_z.append(z.tolist())
  raw_a = np.array(temp_a)
  raw_z = np.array(temp_z)
